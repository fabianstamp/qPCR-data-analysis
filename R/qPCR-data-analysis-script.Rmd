---
title: "Analysis of gene expresssion data from qPCR in R"
author: "FStamp"
date: "Last time modified: `r format(Sys.time(), '%B %d, %Y')`"
output: 
  html_document:
    toc: true
    toc_depth: 3    
...
This is a simple R Markdown script for researchers interested in transitioning their qPCR analysis from GUI based platforms such as Prism or Excel to R. An example data set is provided (**qPCR_data.xlsx**) containing experimental data comparing the effect of a gene KO on the expression of 5 genes of interest.


## What information is required?    

The following information is required for the analysis (see **qPCR_data.xlsx**):  
...
2. **Data analysis**: this includes (A) calculation of normalized gene expression, (B) calculation of ∆∆Ct and Log2FFoldChange per sample and (C) selection of adequate statistics using a decision tree.  
...
Merge the *condition*, *replicates*, *samples*, *genes* and *CT* data frames into one data frame (df). "NTP" is used here to refer to "no template" containing control wells that are not used for the expression analysis.
...
```
### (2) Calculate the geometric mean of the reference genes
...
All tests will be run on the data, however the comparison decision tree will select and highlight the appropriate test that meets the requirements (distribution of data and variance).  
...
This requires some cheating, since the stat_pvalue_manual() is usually used with predefined test functions. To use this function we are creating a data frame that looks like the output of these functions but contains our p values and p star symbols.
...
Defining a custom theme that can be applied to the plots. There are countless options that are not included and can be found online.  
...
### Plotted results with Log2FoldChange  
...
The gene of interest is selected with the filter() function. IMPORTANT: Make sure to match your selection with the gene name in the stat_pvalue_manual() call that selects the fitting statistics results.  
Note: "Condition_label" containing the N number is used instead of "Condition" for the X-axis labels here.
